...
// BETA-3: dedicated emulator runner for tests/firestore.beta.rules.test.js,
// mirroring scripts/run-firestore-v2-tests.mjs's exact pattern. This
// ruleset is never deployed to production (firebase.json/firebase deploy
// only ever ship firestore.rules), so there is no parity guard here -
// firestore.beta.rules has no production counterpart to drift from. This
// is purely a local emulator run; it never touches the real
// tracktozero-beta Firebase project.

const workspaceRoot = resolve(process.cwd());
const configHome = resolve(workspaceRoot, ".firebase-config-beta");
const firebaseBinDir = resolve(workspaceRoot, ".firebase-bin-beta");
...
const command = "node --test --test-concurrency=1 tests/firestore.beta.rules.test.js";

const runSuiteAgainst = async (rulesFile) => {
  const firestorePort = await findOpenPort(8080);
  const tempFirebaseConfigPath = resolve(configHome, "firebase.beta.test.json");
  ...
  console.log(`\n=== Running beta allowlist Firestore suite against ${rulesFile} (local emulator only) ===\n`);
...
const status = await runSuiteAgainst("firestore.beta.rules");
if (status !== 0) {
  console.error("\nFAILED: the beta allowlist Firestore rules suite failed against firestore.beta.rules.");
  process.exit(status);
}

console.log("\nBeta allowlist rules suite: PASS.\n");
process.exit(0);
