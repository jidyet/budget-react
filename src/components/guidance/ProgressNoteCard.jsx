export default function ProgressNoteCard({ palette, note }) {
  ...
  const eyebrow =
    note?.id === "months-sooner" ? "Payoff path" :
    note?.id === "shared-progress" ? "Shared progress" :
    note?.id === "strong-week" ? "Momentum" :
    note?.id === "milestone" ? "Milestone" :
    "Nice work";
  ...
  return (
    <div style={{ background:`linear-gradient(135deg, ${toneColor}14, ${c.surf} 48%, ${c.surf2})`, border:`1px solid ${c.border}`, borderRadius:20, padding:"16px 16px", display:"grid", gap:8 }}>
      <div style={{ fontSize:10, fontWeight:900, letterSpacing:"0.1em", textTransform:"uppercase", color:c.muted }}>{eyebrow}</div>
