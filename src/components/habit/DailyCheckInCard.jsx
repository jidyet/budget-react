export default function DailyCheckInCard({ palette, checkIn, onAction }) {
  ...
  return (
    <div style={{ background:`linear-gradient(135deg, ${accent}14, ${c.surf} 38%, ${c.surf2})`, border:`1px solid ${accent}30`, borderRadius:22, padding:"14px 18px", display:"grid", gap:8, boxShadow:`0 16px 34px ${accent}12` }}>
      ...
      <div style={{ fontSize:18, fontWeight:900, color:c.tx, lineHeight:1.08 }}>{checkIn?.title || "Keep going"}</div>
      <div style={{ fontSize:14, fontWeight:800, color:accent, lineHeight:1.28 }}>{checkIn?.body || "One small action today."}</div>
      <div style={{ fontSize:12, color:c.tx2, lineHeight:1.45 }}>{checkIn?.detail || "You only need one move today."}</div>
      <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
        ...
        <button
          ...
          style={{
            ...
            borderRadius:999,
            border:"none",
            background:`linear-gradient(135deg, ${c.wa}, #ff5f74)`,
            color:"#ffffff",
            fontSize:12,
            fontWeight:900,
            cursor:"pointer",
            boxShadow:`0 10px 24px ${c.wa}33`,
          }}
        >
          Take action
        </button>
