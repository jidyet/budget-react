...
// Expected schedules generated by the payoff engine use "Mon YYYY", while
// older repository fixtures and some persisted schedules use "YYYY-MM". Plan
// status must compare the underlying month, never the display string.
const checkpointPeriodKey = (value) => {
  const text = String(value || "").trim();
  const isoMatch = text.match(/^(\d{4})-(\d{1,2})(?:-\d{1,2})?$/);
  if (isoMatch) return `${isoMatch[1]}-${isoMatch[2].padStart(2, "0")}`;

  const labelMatch = text.match(/^([A-Za-z]{3,9})\s+(\d{4})$/);
  const month = labelMatch ? MONTH_NAMES[labelMatch[1].slice(0, 3).toLowerCase()] : "";
  return month ? `${labelMatch[2]}-${month}` : "";
};

const getExpectedCheckpointForPeriod = (checkpoints = [], period) => {
  const requestedPeriod = checkpointPeriodKey(period);
  const normalized = checkpoints
    .map((checkpoint) => ({ checkpoint, period: checkpointPeriodKey(checkpoint?.period) }))
    .filter((item) => item.period)
    .sort((a, b) => a.period.localeCompare(b.period));
  if (!normalized.length || !requestedPeriod) return normalized[0]?.checkpoint || null;

  const exact = normalized.find((item) => item.period === requestedPeriod);
  if (exact) return exact.checkpoint;
  return normalized.filter((item) => item.period <= requestedPeriod).at(-1)?.checkpoint
    || normalized[0].checkpoint;
};
