export default function MoreDrawer({ open, page, palette, isMobile, onClose, navigateTo, founderOpsEnabled = false }) {
  if (!open) return null;
  const c = palette;
  const items = [
  ...
  ].filter((item) => founderOpsEnabled || item.id !== "founder");
  ...
  return (
    <>
      ...
      >
        {isMobile && <div style={{ width: 40, height: 4, borderRadius: 999, background: c.border2, margin: '0 auto 4px' }} />}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
          <span style={{ fontSize: 17, fontWeight: 800, color: c.tx }}>More</span>
